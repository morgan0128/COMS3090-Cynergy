...
public class ChatServer2 {
    ...
    public void onOpen(Session session, @PathParam("username") String username) throws IOException {
        ...
        if (usernameSessionMap.containsKey(username)) {
            ...
            //session.close();
            // modification: generate new username for user
            int i = 2;
            useUsername = username + i;
            while (usernameSessionMap.containsKey(useUsername) && i < 1000) {
                i++;
                useUsername = username + i;
            }
            if (usernameSessionMap.containsKey(useUsername)){
                session.close();
                return;
            }
        ...
        sessionUsernameMap.put(session, useUsername);

        // map current username with session
        usernameSessionMap.put(useUsername, session);

        // send to the user joining in
        sendMessageToPArticularUser(useUsername, "Welcome to the chat server, " + useUsername + "; this will help prepare you for demo 3.");

        // send to everyone in the chat
        broadcast("User: " + useUsername + " has Joined the Chat. Welcome to demo 3!");
    }
    /**
